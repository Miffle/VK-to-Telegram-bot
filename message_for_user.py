            ...
            if (current_conversation["unread_count"]) != 0:
                ...
                if messages is not None:
                    ...
                    while current_message_num < size:
                        ...
                        if conversation_type == 'chat':
                            bot.send_message(message, text=f"*{current_message.sender}*", reply_markup=markup,
...
def send_attachments(current_message: message_for_user_def.Message, bot, message, markup):
    for attachment in current_message.attachments:
        if attachment.attachment_type == 'video':
            bot.send_message(message, text=attachment.attachment_link, reply_markup=markup, disable_notification=True)
        elif attachment.attachment_type == 'photo':
            bot.send_photo(message, photo=attachment.attachment_link, reply_markup=markup, disable_notification=True)
        elif attachment.attachment_type == 'audio':
            bot.send_audio(message, audio=attachment.attachment_link, reply_markup=markup, disable_notification=True)
            ...
            bot.send_voice(message, voice=attachment.attachment_link, reply_markup=markup, disable_notification=True)
